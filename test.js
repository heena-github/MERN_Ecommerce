...
//  const ans = categories.map((c)=>({value:c,label:c.split('-').join(' '),checked:false}))
//   console.log(ans)

const brands =products.map((p)=>p.brand)
 const allbrands = [ ...new Set(brands)]
 const ans = allbrands.map((c)=>({value:c,label:c,checked:false}))
  console.log(ans)